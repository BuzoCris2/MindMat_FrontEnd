...
export class GrowingTreeComponent implements OnChanges {
  ...
  currentTreeSize: { width: string, height: string } = { width: '200px', height: '500px' }; // Tamaño del árbol
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['growthStage'] && !changes['growthStage'].firstChange) {
      ...
      // Espera hasta que la animación de desaparecer haya terminado antes de cambiar la imagen
      setTimeout(() => {
        this.updateTreeImage();
        this.updateTreeSize(); // Aseguramos actualizar el tamaño del árbol
  ...
  updateTreeImage(): void {
    const imageIndex = this.growthStage + 1; // Las imágenes comienzan desde arbol-1
    ...
    if (this.growthStage >= 4) {
      switch (this.growthStage) {
        ...
        case 6:
          ...
          break;
        case 7:
          this.currentTreeSize = { width: '350px', height: '850px' };
          break;
        case 8:
          this.currentTreeSize = { width: '400px', height: '950px' };
          break;
      }
    } else {
